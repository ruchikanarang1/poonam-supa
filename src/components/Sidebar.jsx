...
export default function Sidebar() {
    ...
    return (
        <aside style={{
            width: '245px', flexShrink: 0,
            background: '#fafafa', borderRight: '1px solid #e8eaed',
            padding: '1rem 0.5rem',
            display: 'flex', flexDirection: 'column',
            minHeight: 'calc(100vh - 70px)', overflowY: 'auto'
        }}>

            {/* --- Global --- */}
            {navLink('/', <LayoutDashboard size={18} />, 'Home / Dashboard', isActivePath('/'))}
            {divider()}
            ...
            {isAdmin && (
                <>
                    ...
                    {tabLink('overview', <LayoutDashboard size={16} color={isActiveTab('overview') ? 'var(--color-primary)' : 'gray'} />, 'Dashboard Overview')}
                    {tabLink('products', <Package size={16} color={isActiveTab('products') ? 'var(--color-primary)' : 'gray'} />, 'Manage Products')}
                    {tabLink('orders', <ShoppingBag size={16} color={isActiveTab('orders') ? 'var(--color-primary)' : 'gray'} />, 'View Orders')}

                    {divider()}
                    {sectionLabel('Team & Forms')}
                    {tabLink('team', <Users size={16} color={isActiveTab('team') ? 'var(--color-primary)' : 'gray'} />, 'Team & Roles')}
                    {tabLink('forms', <Settings size={16} color={isActiveTab('forms') ? 'var(--color-primary)' : 'gray'} />, 'Dynamic Forms')}

                    {divider()}
                    {sectionLabel('Logistics')}
                    {tabLink('reconciliation', <GitMerge size={16} color={isActiveTab('reconciliation') ? 'var(--color-primary)' : 'gray'} />, 'Logistics Matcher')}

                    {divider()}
                    {sectionLabel('Tickets')}
                    {tabLink('ticket_builder', <ClipboardList size={16} color={isActiveTab('ticket_builder') ? 'var(--color-primary)' : 'gray'} />, 'Ticket Categories')}
                    {tabLink('ticket_reviews', <Ticket size={16} color={isActiveTab('ticket_reviews') ? 'var(--color-primary)' : 'gray'} />, 'Ticket Reviews')}

                    {divider()}
                    {sectionLabel('Procurement')}
                    {tabLink('suppliers', <Building2 size={16} color={isActiveTab('suppliers') ? 'var(--color-primary)' : 'gray'} />, 'Supplier Database')}
                </>
            )}
        </aside>
    );
